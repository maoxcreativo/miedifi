...
NSString *const kLinphoneMsgNotificationAppGroupId = @"group.org.miedifi.phone.msgNotification";
...
- (void)migrationAllPre {
	...
	if ([self lpConfigBoolForKey:@"migration_xmlrpc"] == NO) {
		[self lpConfigSetString:@"https://subscribe.linphone.org:444/wizard.php"
		 forKey:@"xmlrpc_url"
		 inSection:@"assistant"];
		[self lpConfigSetString:@"sip:rls@pl.miedifi.com" forKey:@"rls_uri" inSection:@"sip"];
...
- (void)migrationLinphoneSettings {
		...
		while (accounts)
		{
			...
			if (addr &&
				...
				linphone_account_params_set_quality_reporting_collector(
										      newAccountParams, "sip:voip-metrics@pl.miedifi.com;transport=tls");
	...
	if ([self lpConfigBoolForKey:@"lime_migration_done"] == FALSE) {
		...
		while (accounts)
		{
			if (!strcmp(linphone_account_params_get_domain(linphone_account_get_params((LinphoneAccount *)accounts->data)),"pl.miedifi.com")) {
	   ...
	   while (accounts) {
		   ...
		   if (strcmp(appDomain.UTF8String, linphone_account_params_get_domain(newAccountParams)) == 0) {
			   ...
			   if (!linphone_account_params_get_conference_factory_uri(newAccountParams)) {
				   linphone_account_params_set_conference_factory_uri(newAccountParams, "sip:conference-factory@pl.miedifi.com");
				   linphone_account_set_params(account, newAccountParams);
			   }
			   
			   if (!linphone_account_params_get_audio_video_conference_factory_address(newAccountParams)) {
				   NSString *uri = [self lpConfigStringForKey:@"default_audio_video_conference_factory_uri" withDefault:@"sip:videoconference-factory2@pl.miedifi.com"];
...
- (void)enableLinphoneAccountSpecificSettings {
	...
	while (accountsList) {
		...
		if (strcmp(linphone_address_get_domain(currentAddress), "pl.miedifi.com") == 0) {
